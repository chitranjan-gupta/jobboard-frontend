...
export default function CompaniesPage() {
    ...
    // Map companies and augment with job counts from the jobs array
    const augmentedCompanies = useMemo(() => {
        return companies.map(company => {
            ...
            return {
                ...
                types: [...new Set(companyJobs.map(j => j.locationType))],
            };
        }).sort((a, b) => b.jobCount - a.jobCount);
    }, [companies, jobs]);

    const filtered = augmentedCompanies.filter(c =>
        c.name.toLowerCase().includes(query.toLowerCase())
    );

    const handleCompanyClick = (companyName) => {
        // Navigate to homepage with exact company filter
            ...
            <main className="flex-1 pb-12">
                ...
                <div className="text-center py-12">
                    ...
                    <p className={`text-lg mb-8 ${isTronMode ? 'text-slate-400 font-mono' : 'text-slate-600'}`}>
                        <span className={isTronMode ? 'text-neon-cyan' : 'text-primary font-bold'}>{augmentedCompanies.length}</span> {isTronMode ? 'corporations actively signaling' : 'companies hiring right now'}
                    </p>
                        ...
                        {filtered.map(company => (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-4 mb-4 relative z-10">
                                    {company.logo ? (
                                        <Image unoptimized width={56} height={56} src={company.logo} alt={company.name}
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className={`text-sm mt-0.5 ${isTronMode ? 'font-mono text-neon-cyan/80' : 'text-slate-500 font-medium'}`}>
                                            {company.jobCount} {isTronMode ? 'Active Node' : 'Open Job'}{company.jobCount !== 1 ? 's' : ''}
                                        </p>
                                    </div>
                                </div>
